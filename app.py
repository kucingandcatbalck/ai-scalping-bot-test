...
st.set_page_config(
    page_title="Deep AI Swing Pro v12.1 (Balanced Sniper)", 
    layout="wide", 
...
def scan_global_market_native():
    try:
        ...
        # DILONGgARKAN: Konsolodasi dan koreksi ringan tetap diberi skor netral/positif agar bot mau melirik koin
        if close > ema20 and ema20 > ema50: return "STRONG BULLISH 🚀", 1.5
        elif close > ema20: return "BULLISH 📈", 1.0
        elif close < ema20 and ema20 < ema50: return "KOREKSI/SIDEWAYS ⚖️", 0.5
        else: return "KONSOLIDASI ⚖️", 0.5
    except: return "NORMAL ⚖️", 0.5
...
def analyze_coin_sniper(symbol):
    try:
        ...
        return {
            ...
            'rsi_15': rsi_15, 'macd_growth': macd_growth, 'atr_pct': atr_pct
        }
    except Exception as e: return None

with st.sidebar:
    st.title("🧠 Deep AI Brain v12.1")
    st.write("Mode: **Balanced Sniper (15m)** 🎯")
    ...
    if st.button("▶️ AKTIFKAN AI", use_container_width=True):
        ...
        push_log("V12.1 Balanced Sniper Aktif! Lebih fleksibel mencari peluang.", "system")
        st.rerun()
...
st.header("⚡ AI Swing Pro (Balanced Sniper Telemetry)")
...
with col_right:
    st.markdown("**🔍 15m Balanced Matrix (Real-Time)**")
...
def autonomous_trading_loop():
    ...
    macro_status, macro_score = scan_global_market_native()
    push_scan(f"🔄 Balanced Scan 15m... | Makro: {macro_status}", "heartbeat")
    scan_container.markdown(f'<div class="scan-container">{"".join(st.session_state["scan_reports"])}</div>', unsafe_allow_html=True)
    ...
    for sym, pos in list(st.session_state['active_trades'].items()):
        try:
            ...
            trailing_trigger_pct = max(0.7, pos.get('atr_pct', 1.0) * 0.5)
            ...
            if pnl_pct >= trailing_trigger_pct and not pos.get('trailing_breakeven_active', False):
                ...
                push_log(f"🛡️ AUTO-LOCK {sym}: SL diamankan ke modal + fee.", "warn")

            dynamic_tp = pos['entry'] * (1 + (max(1.2, pos.get('atr_pct', 1.0) * 1.0) / 100))
    ...
    # 2. BALANCED SCANNER (Lebih fleksibel mencari entry)
    for sym in WATCHLIST:
        ...
        vote_local_trend = 1.5 if (close_price > data['ema20_15']) else 0.8
        vote_momentum = 1.5 if (30 <= rsi_15 <= 60) else 0.2  # Rentang RSI dilebarkan agar lebih mudah masuk
        vote_volatility = 1.3 if close_price <= data['bb_lower_15'] * 1.025 else 0.5
        
        total_score = (macro_score) + (vote_local_trend * W['trend_agent']) + (vote_momentum * W['momentum_agent']) + (vote_volatility * W['volatility_agent'])
        
        # DILONGRGARKAN: Threshold diturunkan dari 3.8 menjadi 3.0 agar lebih sering melakukan order
        threshold = 3.0 
        ...
        if total_score >= threshold and len(st.session_state['active_trades']) < (1 if total_eq < 10 else 2) and usdt_free >= 2.5:
            ...
            if ob_ratio < 0.65: # Sedikit dilonggarkan juga
                push_scan(f"🛑 HOLD {sym} -> Orderbook Asks Dominan", "blocked")
                scan_container.markdown(f'<div class="scan-container">{"".join(st.session_state["scan_reports"])}</div>', unsafe_allow_html=True)
                continue
                
            push_scan(f"🎯 {sym} TERKUNCI! -> 🚀 BALANCED ENTRY APPROVED!", "passed")
            scan_container.markdown(f'<div class="scan-container">{"".join(st.session_state["scan_reports"])}</div>', unsafe_allow_html=True)
            try:
                ...
                push_log(f"🔫 BALANCED SHOT {sym} @ {close_price:.4f} | Size: ${alloc:.2f}", "system")
                st.rerun()
                break
            except Exception as e:
